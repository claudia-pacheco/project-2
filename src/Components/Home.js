...
function Home() {
  ...
  function generateCharacter() {
    const classConst = classes.results[Math.floor(Math.random() * classes.results.length)]
    updateRandomClass(classConst)
    fetchClassInfo(classConst);
    displayingClassImg(classConst);
    // updateRandomClass(randomClass)
    const raceConst = races.results[Math.floor(Math.random() * races.results.length)]
    updateRandomRace(
      raceConst
    );
    fetchRaceInfo(raceConst);
    // updateRandomRace(randomRace)
    const skillConst = skills.results[Math.floor(Math.random() * skills.results.length)]
    updateRandomSkill(
      skillConst
    );
