...
import halfling from "../assets/MaleLightfootHalfling_Rogue-removebg-preview.png";
...
function RacesDisplay() {
  ...
  function displayImage(raceDisplay) {
    ...
    if (raceDisplay.index === "half-elf") {
      return halfelf;
    }
    if (raceDisplay.index === "half-orc") {
      return halforc;
    }
    ...
    <>
      ...
      {raceDisplay.length !== 0 ? (
        raceDisplay.results.map((elem) => (
          <div className="races-div" key={elem.index}>
            <img src={displayImage(elem)}></img>
            <p className="races">{elem.name}</p>
          </div>
        ))
      ) : (
        <p>... loading</p>
